...
function cfr_plugin_row_js(){
    ...
    if ( $current_page->base == 'plugins' ) {
        ...
        wp_localize_script( 'cfr_plugin_row_js', 'cfr_sub_nonce', array('nonce' => wp_create_nonce("cfr_subscribe")) );
    }
}
...
function cfr_plugin_row( $plugin_meta, $plugin_file, $plugin_data, $status ) {
    ...
    if ( $plugin_file == "contact-form-ready/contact-form-ready.php") {
        ...
        if (!$check) {
            ...
            $ret .= '<input type="text" name="cfr_signup_newsletter" id="cfr_signup_newsletter" value="'.esc_attr(get_option( 'admin_email' )).'"></option>';
